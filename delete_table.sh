...
function delete_from_table() {
    ...
    if [ -z "$tables" ]; then
        ...
        select table in "${tables[@]}";do
            if [ -n "$table" ]; then      #[ -n "$table" ]: checks if the variable $table is not an empty string
                if [ -e "$table" ]; then  # [ -e "$table" ] checks if a file or directory with the name specified in the variable $table exists
                    if [ "$(awk 'NR >= 4 { print NR - 3 }' "$table" | wc -l)" -eq 0 ]; then
                        ...
                        select choice in "Delete Specified Row" "Delete All"; do
                            case $choice in
                                "Delete Specified Row")
                                    ...
                                    read -p "Are you sure you want to delete row? (yes/no)" confirm
            ...
            read -p "Do you want to drop another table? (yes/no)" drop_another
