...
function select_from_table() {
    while true; do
        ...
        read -p "#-> Enter the table name to select from (enter 'exit' to quit): " insert_table_name
...
select_by_primary_key() {
    primary_key_column_index=$(awk -F'|' 'NR==2 {for (i=1; i<=NF; i++) if ($i == "yes") print i}' "$insert_table_name")
    ...
    echo "Executing: SELECT * FROM $insert_table_name WHERE ${column_names[$((primary_key_column_index-1))]} = $primary_key;"
    echo "Columns: ${column_names[@]}"
