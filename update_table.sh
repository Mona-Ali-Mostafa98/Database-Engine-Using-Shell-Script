...
function update_table() {
   ...
   while true; do
       ...
       if [ ! -e "$update_table_name" ]; then
           echo "Table $update_table_name does not exist. Please enter a valid table name ❌."
           continue
       fi
       ...
       if [ ${#primary_key_values[@]} -eq 0 ]; then
           echo "File $update_table_name is empty."
           continue
       fi
       ...
       while true; do
           ...
           case "${column_types[update_column_number-1]}" in
               "int")
                   ...
                   ;;
               "str")
                  if [[ "$updated_value" =~ ^[a-zA-Z0-9_]+$ && ${#updated_value} -ge 2 ]]; then
                      break
                  else
       ...
       echo "Record updated successfully in table $update_table_name ✅."
       break
   done
}